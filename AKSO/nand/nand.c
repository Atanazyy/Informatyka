...
typedef struct pair
{
    nand_t *gate;     // gate that is connected
    node_t *position; // where in that gate's list is information remembered
} pair;
...
struct nand
{
    ...
    bool value;
    unsigned path_len;
    bool const *signal; // NULL if gate is not representing boolean signal
    unsigned k;
    list_t *out; // list of gates that this gate is connected to
    pair in[];
};
...
void nand_delete(nand_t *g)
{
    ...
    for (unsigned i = 0; i < g->k; i++)
    {
        ...
        {
            nand_delete(g->in[i].gate); // removing gates representing signals
        }
    }
...
{
    errno = 0;
    if (g_in == NULL || g_out == NULL || k >= g_in->k)
    {
        errno = EINVAL;
        return -1;
    }
...
{
    errno = 0;
    if (s == NULL || g == NULL || k >= g->k)
    {
        errno = EINVAL;
        return -1;
    }
    nand_t *new_nand = nand_new(0); // creating artificial gate representing signal with 0 inputs
    if (new_nand == NULL)
    {
        errno = ENOMEM;
        return -1;
...
void *nand_input(nand_t const *g, unsigned k)
{
    ...
    else if (g->in[k].gate->signal == NULL)
    {
        return g->in[k].gate;
    }
    else
    {
        return (void *)g->in[k].gate->signal;
    }
}
...
static ssize_t evaluate_single(nand_t *g, list_t *all_gates)
{
    ...
    if (g->signal != NULL) // gate is representing boolean signal
    {
        g->value = *g->signal;
        g->path_len = 0;
        return 0;
...
ssize_t nand_evaluate(nand_t **g, bool *s, size_t m)
{
    ...
    list_t *all_gates = init(); // using list just to hold pointers to the visited gates
    if (all_gates == NULL)
    {
        errno = ENOMEM;
        return -1;
