...
typedef struct kartka //nazwy zmiennych z wejscia tak jak w tresci zadania
{
    char typ;
    int k;
...
int licz(kartka tab[], int k, double x, double y) //liczy wynik dla punktu (x, y) w kartce k
{
    ...
    if(str == 1) 
        return 0; //punkt po prawej stronie
    if(str == 0) 
        return licz(tab, tab[k].k, x, y); //punkt na prostej
    return licz(tab, tab[k].k, x, y) + licz(tab, tab[k].k, creal(Pp), cimag(Pp)); //punkt po lewej stronie
}
